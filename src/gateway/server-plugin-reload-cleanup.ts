...
export function createPluginReloadCleanup({
  ...
  const drainWithDeadline = async (
    ...
    reportStatus({
      ...
      reason:
        phase === "recovering"
          ? "Waiting for admitted work before restoring the previous plugin runtime."
  ...
  return {
    attempt,
    prepareResourceHandoff: (nextRegistry: PluginRegistry, requestedIds: ReadonlySet<string>) => {
