...
suite.define(() => {
  it("commits a streamed delta burst in frame-bound transcript batches", async () => {
    await suite.withPage(createControlUiE2eContextOptions(), async ({ page }) => {
      ...
      // Drain the initial roster publication before measuring stream-only updates.
      await page.evaluate(async () => {
        const sidebar =
        ...
        if (!sidebar.sessionData.sessionsResult || sidebar.sessionData.sessionsLoading) {
          throw new Error("sidebar session roster is not ready");
        }
