...
beforeEach(async () => {
  state = await createOpenClawTestState({ prefix: "openclaw-codex-skill-usage-", applyEnv: true });
  skillFile = await state.writeText("skills/daily-brief/SKILL.md", "# Daily brief\n");
  resetGlobalHookRunner();
  ...
  unregisterUsage = registerSkillUsageTracking({ env: state.env });
});
...
function usageRows() {
  return openOpenClawStateDatabase({ env: state.env })
    .db.prepare(
...
describe("persistent skill usage through registered Codex dynamic tools", () => {
  ...
  it.each(["skills/unknown/SKILL.md", "README.md"])(
    ...
    async (filePath) => {
      const otherFile = await state.writeText(filePath, "Other file\n");
      const { call } = createBridge();
