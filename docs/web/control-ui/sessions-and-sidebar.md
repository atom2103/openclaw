...
The top row becomes a neutral workspace header with the configured Gateway display name, or **OpenClaw**, and a static OpenClaw mark with the same size and spacing as the agent avatar. Its menu keeps the agent switcher, followed by **Every agent**, **New agent**, and **Agent settings**. Choose an agent to leave team mode with that agent selected and restore the agent chip. **Every agent** opens the Agents home. The **Sessions from every agent** toggle is checked while team mode is on. Documentation, help, community, and changelog links remain under **Help** in the account menu at the bottom of the sidebar. The sidebar header toolbar contains collapse, search, session filters, and **+** controls.
